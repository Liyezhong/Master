...
void CModule::CopyFromOther(const CModule &Other)
{
    ...
    m_DateOfProduction = OtherModule.GetDateOfProduction();
...
{    
    ...
    // Serial Number
    if (!DataManager::Helper::ReadNode(XmlStreamReader, "SerialNumber")) {
    ...
    //Operating Hrs
    if (!DataManager::Helper::ReadNode(XmlStreamReader, "OperatingHours")) {
...
bool CModule::SerializeContent(QXmlStreamWriter &XmlStreamWriter, bool CompleteData)
{
    ...
    {
        ServiceDataManager::CSubModule* p_SubModule = GetSubModuleInfo(i);
        if (p_SubModule != NULL) {
...
QDataStream& operator >> (QDataStream& InDataStream, CModule& Moduleinfo)
{
    ...
    if (!DataManager::Helper::ReadNode(XmlStreamReader, QString("Module"))) {
