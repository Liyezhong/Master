...
/****************************************************************************/
void ImportExportThreadController::ResetTheStationPSContainers(const QString TypeOfImport) {
    // get the program sequence list from the data manger
    DataManager::CProgramSequenceList* p_ProgramSeqList = m_DataManager.GetProgramSequenceList();
    if (p_ProgramSeqList != NULL) {
        // reset the program sequence list and make the start capable status and BLG Result
        for(int ProgramSeqCount = 0; ProgramSeqCount < p_ProgramSeqList->GetNumberOfPrograms();
            ProgramSeqCount++) {
            DataManager::CProgramSequence* p_ProgramSequence = const_cast<DataManager::CProgramSequence*>
                    (m_DataManager.GetProgramSequenceList()->GetProgramSequenceStep(ProgramSeqCount));
            // reset the program sequence data
            if (p_ProgramSequence != NULL) {
                p_ProgramSequence->SetStartCapableStatus(false);
                //p_ProgramSequence->SetBLGResult("-1");
            }
        }
    }
    // get the station list from the datamanager
    DataManager::CDashboardDataStationList* p_StationList = m_DataManager.GetStationList();
    if (p_StationList != NULL) {
        // reset the station list
       /* for(int StationCount = 0; StationCount < p_StationList->GetNumberOfStations(); StationCount++) {
            DataManager::CDashboardStation* p_Station = const_cast<DataManager::CDashboardStation*>
                    (m_DataManager.GetStationList()->GetStation(StationCount));;
            // reset the stations data
            if (p_Station != NULL) {
                bool ResetReagent = true;
                // check the type of Import
                if (TypeOfImport.compare("Leica", Qt::CaseInsensitive) == 0) {
                    // dont reset the loader stations reagent IDs
                    if (p_Station->GetStationID().at(0) == 'L') {
                        ResetReagent = false;
                    }
                }
                // check the reset flag of the reagent
                if (ResetReagent) {
                    p_Station->SetReagentID("-1");
                }
                p_Station->SetNumberOfActualSlides(0);
                p_Station->SetProgramID("-1");
                p_Station->SetStepID("-1");
            }
        }*/
    }
}
