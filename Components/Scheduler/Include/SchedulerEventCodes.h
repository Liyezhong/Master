...
const quint32 EVENT_SCHEDULER_POWER_FAILURE_NOREAGENT_STEP = SCHEDULER_BASE_ID + 0x0301; ///< Reheating for 200,260,203, 2*1 or 2*7 step.
...
const quint32 EVENT_SCHEDULER_LOCK_RETORT_LID = SCHEDULER_BASE_ID + 0x0313; ///< 67175187, Lock the retort, then press ok button.
...
const quint32 EVENT_SCHEDULER_START_PROGRAM_WITH_EXPIRED_REAGENT = SCHEDULER_BASE_ID + 0x0339; ///< Start one program when expired reagents
const quint32 EVENT_SCHEDULER_PAUSE_ENDTIME_UPDATE = SCHEDULER_BASE_ID + 0x033E; ///< Because user paused the program, the program end time will be updated to %1
