...
//    //m_TickTimer.start();
    if(QFile::exists("RetortConfiguration.txt"))
    {
        ...
        if (Test.open(QIODevice::ReadOnly | QIODevice::Text))
        {
            ...
            if(!in.atEnd())
            {
                ...
                if (!RetortName.isEmpty())
                {
                    ...
                    m_SchedulerStateHandlerList.insert(RetortName, QSharedPointer<CSchedulerStateHandler>(new CSchedulerStateHandler(RetortName, this, mp_DataManager)));
                }
            }
            Test.close();
        }
    ...
    //m_SchedulerStateHandlerList.insert(QString("Retort2"), QSharedPointer<CSchedulerStateHandler>(new CSchedulerStateHandler(QString("Retort2"), this, mp_DataManager)));
    m_TickTimer.start();
}
